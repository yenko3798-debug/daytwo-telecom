...
type DetectorOptions = {
  ...
  onResult: (payload: { sessionId: string; channelId: string; voicemail: Record<string, any> }) => Promise<void> | void;
};
...
function normalizeSettings(metadata?: DetectionMetadata | null): DetectionSettings {
...
async function recordSnippet(client: any, channel: any, sessionId: string, sampleSeconds: number) {
  ...
  const buffer = (await client.recordings.getStoredFile({ recordingName })) as Buffer;
  await client.recordings.deleteStored({ recordingName }).catch(() => {});
  return buffer;
}

function analyzeBeep(buffer: Buffer, settings: DetectionSettings): BeepResult {
  try {
    const wav = decode(buffer);
    const channel = wav.channelData?.[0];
    if (!channel) return { detected: false, beepAtMs: null };
...
async function transcribeAudio(buffer: Buffer, recordingName: string, settings: DetectionSettings): Promise<TranscriptResult | null> {
...
function evaluateTranscript(text: string, keywords: string[]) {
  if (!text) {
...
function buildResult(beep: BeepResult, transcript: TranscriptResult | null, settings: DetectionSettings) {
  ...
  if (beep.detected) reasons.push("Beep");
  if (transcriptEval.hits.length) reasons.push(`Transcript: ${transcriptEval.hits.join(", ")}`);
  return {
    detected,
...
export function startVoicemailDetection(options: DetectorOptions) {
